...
class K6CP_Customize_Control_Size extends K6CP_Customize_Control_Base {
	...
	protected function js_tpl() {
			/*<div class="switch-toggle k6-switch switch-2">
				<input id="k6cp_size_dynamic-choice_{{ data.id }}" type="radio" value="custom" name="_customize-k6cp_size_dynamic-custom-{{ data.id }}" <# if (data.value===data.isExpr) { #>checked<# } #>>
				<label for="k6cp_size_dynamic-custom-{{ data.id }}" onclick=""><?php _e( 'Custom', 'pkgtextdomain' ); ?></label>
				<input id="k6cp_size_dynamic-choice_{{ data.id }}" type="radio" value="expr" name="_customize-k6cp_size_dynamic-expr-{{ data.id }}" <# if (data.value!==data.isExpr) { #>checked<# } #>>
				<label for="k6cp_size_dynamic-expression-{{ data.id }}" onclick=""><?php _e( 'Expression', 'pkgtextdomain' ); ?></label>
				<a></a>
			</div>*/
		?>
		<label>
