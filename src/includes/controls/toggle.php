	...
	protected function js_tpl() {
		...
		<label>
			<# if (data.label) { #>
				...
				<input type="checkbox" name="_customize-k6_toggle-{{ data.id }}" value="{{ data.value }}" <# var a = data.attrs; for (var key in a) { if (a.hasOwnProperty(key)) { #>{{ key }}="{{ a[key] }}" <# } } #> <# if (data.value) { #>checked<# } #>>
