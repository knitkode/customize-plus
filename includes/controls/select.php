			...
			if ( is_array( $input_decoded ) ) {
				foreach ( $input_decoded as $key ) {
					if ( ! isset( $control->choices[ $key ] ) ) {
						$validity->add( 'wrong', sprintf( esc_html__( 'The value %s is not selectable.', 'kkcp' ), $key ) );
					}
				}
			} else {
				$validity->add( 'wrong', sprintf( esc_html__( 'The value %s must be a well formed array.', 'kkcp' ), $value ) );
			}
		} else {
			if ( ! isset( $control->choices[ $value ] ) ) {
				$validity->add( 'not_a_choice', sprintf( esc_html__( 'The value %s is not an allowed selection.', 'kkcp' ), $value ) );
			}
		}
		return $validity;
	}
